...
contains
  ...
  subroutine Photosynthesis ( bounds, fn, filterp, &
    ...
    associate(                                                       & 
        ...
        ! Kirk Wythers 01/15/18 added: allocate new variables for pft parameters
        ! needed for respiration acclimation
         br_mr_r0   => veg_vp%br_mr_r0                         , & ! Input:  [real(r8) (:)   ]  intercept  of respiration acclimation as (umol CO2 / m^2 leaf / s)
         br_mr_r1   => veg_vp%br_mr_r1                         , & ! Input:  [real(r8) (:)   ]  leaf respiration nitrogen modification as (umol CO2 / m^2 leaf / s / g N / m^2 LAI)
         br_mr_r2   => veg_vp%br_mr_r2                         , & ! Input:  [real(r8) (:)   ]  leaf respiration temperature term(umol CO2 / m^2 leaf / °C)
         br_mr_nl   => veg_vp%br_mr_nl                         , & ! Input:  [real(r8) (:)   ]  leaf nitrogen (g N / m^2 LAI)
      ...
      do f = 1, fn
         ...
         do iv = 1, nrad(p)
          ...
          !!!!! begin changes for respiration acclimation - krw  

            ! 11/17/15, Kirk Wythers: Added respiration acclimation and
            ! temperature variable Q10 for leaves
            ! The following calculates (umol CO2 / m**2 [leaf] / s) for each
            ! pft(p) at a 10 day mean growth temperature (t10)
            ! and an instantainious leaf temerature (t_veg). From Huntingford,
            ! Atkin, Heskel, Bloomfield, et al. Draft Document 'Case 4, equation
            ! 1'

                ! uncomment for fbq runs - krw 20160311
                ! lmr_z(p,iv) = (br_mr_r0(veg_pp%itype(p)) +
                ! br_mr_r1(veg_pp%itype(p)) * (lnc(p) - 2.13_r8) -
                ! br_mr_r2(veg_pp%itype(p)) * (18.92_r8 - 22.63_r8)) * &
                ! 2._r8 ** ((t_veg(p) - tfrz - 25._r8) / 10._r8) * nscaler

                ! uncomment for fbq10 runs (no acclimation, removed the r2 term)
                ! - krw 20180211
                ! lmr_z(p,iv) = ((br_mr_r0(veg_pp%itype(p)) +
                ! br_mr_r1(veg_pp%itype(p)) * (lnc(p) - 2.13_r8)) * &
                ! 2._r8 ** ((t_veg(p) - tfrz - 25._r8) / 10._r8)) * nscaler

                ! uncomment for vbq10 runs - krw 20180211
                ! lmr_z(p,iv) = ((br_mr_r0(veg_pp%itype(p)) +
                ! br_mr_r1(veg_pp%itype(p)) * (lnc(p) - 2.13_r8) -
                ! br_mr_r2(veg_pp%itype(p)) * ((t10(p) - tfrz) - 22.63_r8)) * &
                ! 2._r8 ** ((t_veg(p) - tfrz - 25._r8) / 10._r8)) * nscaler

                ! uncomment for vbe runs - krw 20180131
                 lmr_z(p,iv) = ((br_mr_r0(veg_pp%itype(p)) +br_mr_r1(veg_pp%itype(p)) * (lnc(p) - 2.13_r8) - (br_mr_r2(veg_pp%itype(p)) *((t10(p) - tfrz) - 22.63_r8))) * &
                 exp(0.1012_r8 * ((t_veg(p) - tfrz) - 25._r8) - 0.0005_r8 *(((t_veg(p) -  tfrz) ** 2) - (25._r8 ** 2)))) * nscaler

               ! uncomment for default runs
               ! lmr_z(p,iv) = lmr25 * ft(t_veg(p), lmrha) * fth(t_veg(p),
               ! lmrhd, lmrse, lmrc)

               !!!!if(nscaler > 1) then
                !!!!   write (iulog,*)'nscaler = ', nscaler
                !!!! if(nscaler > 1) then

                lmr_z(p,iv) = max(0._r8, lmr_z(p,iv)) !!!! prevent respiration from falling below zero (0), take the max of the
                                                      !!!! returned value... if lmr_z is negitive the function will return zero (0).

               !!!!! end changes for respiration acclimation - krw
