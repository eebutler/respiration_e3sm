module VegetationPropertiesType
  ...
  type, public :: vegetation_properties_type
     ...
     ! Kirk Wythers (01/16/18): Added for respiration acclimation
     real(r8), allocatable :: br_mr_r0      (:)   ! parameter for ra equation from Huntingord et al. in progress (umol CO2/m**2[leaf]/s)
     real(r8), allocatable :: br_mr_r1      (:)   ! parameter for ra equation from Huntingord et al. in progress (umol CO2/m**2[leaf]/s)
     real(r8), allocatable :: br_mr_r2      (:)   ! parameter for ra equation from Huntingord et al. in progress (umol CO2/m**2[leaf]/s)
     real(r8), allocatable :: br_mr_nl      (:)   ! parameter for ra equation from Huntingord et al. in progress leaf nitrogen (kg N/kg C)
...
contains
  ...
  subroutine veg_vp_init(this)
    ...
    ! Kirk Wythers (01/16/18): Added for respiration acclimation
    use pftvarcon , only : br_mr_r0, br_mr_r1, br_mr_r2, br_mr_nl    
    ...
    ! Kirk Wythers (01/16/18): Added for respiration acclimation
    allocate(this%br_mr_r0      (0:numpft))        ; this%br_mr_r0     (:)   =nan
    allocate(this%br_mr_r1      (0:numpft))        ; this%br_mr_r1     (:)   =nan
    allocate(this%br_mr_r2      (0:numpft))        ; this%br_mr_r2     (:)   =nan
    allocate(this%br_mr_nl      (0:numpft))        ; this%br_mr_nl     (:)   =nan
    ...
    do m = 0,numpft
       ...
       ! Kirk Wythers (01/16/18): Added for respiration acclimation
       this%br_mr_r0(m)     = br_mr_r0(m)
       this%br_mr_r1(m)     = br_mr_r1(m)
       this%br_mr_r2(m)     = br_mr_r2(m)
       this%br_mr_nl(m)     = br_mr_nl(m)
