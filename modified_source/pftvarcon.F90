module pftvarcon
  ...
  real(r8), allocatable :: br_mr_r0(:)   !umol/m**2[leaf]/s
  real(r8), allocatable :: br_mr_r1(:)   !umol/m**2[leaf]/s
  real(r8), allocatable :: br_mr_r2(:)   !umol/m**2[leaf]/s
  real(r8), allocatable :: br_mr_nl(:)   !leaf nitrogen; kg N/kg C
...
!-----------------------------------------------------------------------
    ...
    ! Kirk Wythers 01/15/18: Allocates array length for new variables needed in
    ! respiration acclimation
    allocate( br_mr_r0      (0:mxpft) )
    allocate( br_mr_r1      (0:mxpft) )
    allocate( br_mr_r2      (0:mxpft) )
    allocate( br_mr_nl      (0:mxpft) )
    ...
    ! Kirk Wythers 01/15/18: added for reading new respiration acclimation
    ! variables from netcdf file, if vaiable is not readable, show error
    call ncd_io('br_mr_r0',br_mr_r0, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(__FILE__, __LINE__))
    call ncd_io('br_mr_r1',br_mr_r1, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(__FILE__, __LINE__))
    call ncd_io('br_mr_r2',br_mr_r2, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(__FILE__, __LINE__))
    call ncd_io('br_mr_nl',br_mr_nl, 'read', ncid, readvar=readv)
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(__FILE__, __LINE__))
